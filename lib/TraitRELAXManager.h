...
class TraitRELAXManager
{
    protected:
		...
		// the below are saved as a data members so that their pointer wouldn't be lost upon closure of init(), a stage in which they cannot be deleted yet
        GeneticCode* gCode_;          
		const BinaryAlphabet* binaryAlphabet_;
		VectorSiteContainer* characterData_;
		Tree* tree_;
		const CodonAlphabet* codonAlphabet_;
		VectorSiteContainer* sequenceData_;
		TransitionModel* characterModel_;
		MixedSubstitutionModelSet* sequenceModel_;
    ...
    public:
        ...
        traitRELAXParameters_(0), traitRELAXLikelihoodFunction_(), nullLogl_(0), alternativeLogl_(0), gCode_(), binaryAlphabet_(), characterData_(),
		tree_(), codonAlphabet_(), sequenceData_(), characterModel_(), sequenceModel_(), rDist_()
        {   
            traitRELAXLikelihoodFunction_ = trm.traitRELAXLikelihoodFunction_; 
            traitRELAXParameters_ = trm.traitRELAXParameters_; 
            gCode_ = trm.gCode_;
			binaryAlphabet_ = trm.binaryAlphabet_;
			characterData_ = trm.characterData_;
			tree_ = trm.tree_;
			codonAlphabet_ = trm.codonAlphabet_;
			sequenceData_ = trm.sequenceData_;
			characterModel_ = trm.characterModel_;
			sequenceModel_ = trm.sequenceModel_;
            rDist_ = trm.rDist_;
        }
    ...
    protected:
        ...
        void setCharacterModel(DRTreeParsimonyScore* mpData);

        // sets initial partition of the branches as input of the sequence model based on a maximum parsimony solution
        void setMpPartition(DRTreeParsimonyScore* mpData);

        // creates the sequence model
        void setSequenceModel(DRTreeParsimonyScore* mpData);
        ...
        void optimizeAlternativeCharacterModelByGrid(map<string, double> &optimalValues, uint verbose = 1, uint gridSize = 10, bool firstCycle = true);
        ...
        void optimizeAlternativeCharacterModelByOneDimBrent(map<string, double> &optimalValues, uint verbose = 1);
        ...
        void optimizeAlternativeCharacterModelByPowell(map<string, double> &optimalValues, uint verbose = 1);
        ...
        Tree* extractMapping(RASiteSimulationResult* simulationData);

        // makes sure that the strcture of the simulated histories complies withh the input tree
        void checkMapping(Tree* mapping);
        ...
        void simulateSequenceEvolution(Tree* mapping);
};
